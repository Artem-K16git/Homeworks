#  Дипломная работа - Капитонов Артем Александрович
...
Инфраструктура должна размещаться в Yandex Cloud и отвечать минимальным стандартам безопасности: запрещается выкладывать токен от облака в git. Используйте инструкцию.  
...
- NAT-шлюз. Для хостов с приватными адресами(2 веб-сервера, elasticsearch) шлюзом является бастион. На нем настроено натирование, через файл cloud-init-bastion.tpl. Сделал так для экономии, дополнительной уязвимости нет, так как работает только на исходящие соединения.  
